...
assert.match(bootstrap,/Version:\s*5\.186\.0/);
assert.match(bootstrap,/define\('TNG_OS_VERSION','5\.186\.0'\)/);
...
assert.match(review,/status\.focus\(\{preventScroll:true\}\)/);
...
// Both exposed dismissal paths move focus to the stable status whose focus
// state now has a visible, non-layout-shifting indicator.
...
assert.equal(h.requests.length,0);
console.log('TN Game OS 5.186.0 Visible Dismissal Focus passed');
