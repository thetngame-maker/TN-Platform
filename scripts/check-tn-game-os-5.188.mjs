...
assert.match(bootstrap,/Version:\s*5\.1(?:8[8-9]|9\d)\.\d+/);
assert.match(bootstrap,/define\('TNG_OS_VERSION','5\.1(?:8[8-9]|9\d)\.\d+'\)/);
...
// The explicit status role preserves the established generic atomic update,
// visible focus handoff, and request-free behavior for both dismissal paths.
...
assert.equal(h.requests.length,0);
console.log('TN Game OS 5.188.0 Semantic Dismissal Status passed');
