...
assert.match(bootstrap,/Version:\s*5\.1(?:8\d|9\d)\.\d+/);
assert.match(bootstrap,/define\('TNG_OS_VERSION','5\.1(?:8\d|9\d)\.\d+'\)/);
...
assert.match(review,/draftReviewComplete(?:Message)?\.textContent = 'All edits on this page are saved\.'/);
...
assert.match(php,/class="tng-adventure-library__draft-complete" data-tng-draft-review-complete/);
assert.match(php,/aria-live="polite" aria-atomic="true"/);
