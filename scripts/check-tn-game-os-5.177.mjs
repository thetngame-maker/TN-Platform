...
assert.match(bootstrap,/Version:\s*5\.177\.0/);
assert.match(bootstrap,/define\('TNG_OS_VERSION','5\.177\.0'\)/);
...
assert.match(review,/field\.name === button\.dataset\.tngDraftReviewType/);
...
assert.equal(button(h,'notes').textContent,'Review 1 note');
...
assert.equal(button(h,'notes').textContent,'Review 2 notes');
assert.equal(button(h,'title').textContent,'Review 1 name');
assert.equal(button(h,'planned_date').textContent,'Review 1 date');
...
h=harness({restored:true});assert.equal(button(h,'notes').hidden,false);assert.equal(button(h,'notes').textContent,'Review 1 note');
