...
assert.match(bootstrap,/Version:\s*5\.182\.0/);
assert.match(bootstrap,/define\('TNG_OS_VERSION','5\.182\.0'\)/);
const review=client.slice(client.indexOf('const draftReview ='),client.indexOf("prepOverview?.addEventListener('click'"));
assert.match(review,/draftReviewCompleteDismiss\?\.addEventListener\('click',[^]*?draftReviewComplete\.hidden = true[^]*?status\.textContent = 'Edit confirmation dismissed\.'[^]*?status\.focus\(\{preventScroll:true\}\)/);
