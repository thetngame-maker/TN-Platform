...
self.addEventListener('message',event=>{
  ...
  if(data.type==='TNG_ADVENTURE_PACK_REMOVE')event.waitUntil((async()=>{const id=adventureId(data.id);if(!id){reply({ok:false,error:'Unknown adventure pack.'});return;}await Promise.all([caches.delete(ADVENTURE_PACK_PREFIX+id),caches.delete(ADVENTURE_PACK_PREFIX+id+'-staging')]);reply({ok:true,id,installed:await adventureStatus([{id,urls:[]}])});})());
