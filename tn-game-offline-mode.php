...
self.addEventListener('message',event=>{
  ...
  const adventureLibrary=async()=>{const packs=[];for(const key of await caches.keys()){if(packs.length>=12)break;if(!key.startsWith(ADVENTURE_PACK_PREFIX)||key.endsWith('-staging'))continue;const id=adventureId(key.slice(ADVENTURE_PACK_PREFIX.length));if(!id)continue;const cache=await caches.open(key);const requests=await cache.keys();const urls=requests.map(request=>request.url).filter(url=>adventureUrl(url));if(!urls.length)continue;let bytes=0;for(const request of requests){try{const response=await cache.match(request);if(response)bytes+=(await response.clone().blob()).size;}catch(error){}}const metadata=await adventureMeta(id);packs.push({id,count:urls.length,urls,bytes,verifiedAt:typeof metadata.verifiedAt==='string'?metadata.verifiedAt:''});}return packs;};
