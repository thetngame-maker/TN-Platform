...
final class TNG_Trip_Data {
    ...
    public static function assets(): void {
        if (is_admin()) return;
        wp_enqueue_script('tng-trip-data', TNG_OS_URL . 'assets/js/trip-data.js', [], '0.1.0', true);
    ...
    public static function is_saved(int $post_id, int $user_id = 0): bool {
        return in_array($post_id, self::ids($user_id), true);
    }
    ...
    public static function toggle(int $post_id, int $user_id): array {
        ...
        return ['saved' => !$saved, 'count' => count($ids), 'ids' => $ids];
    }
    ...
    public static function render_saved(): string {
        ...
        <main class="tng-saved-screen tng-app-shell">
            ...
            <nav class="tng-library-tabs"><a href="<?php echo esc_url(home_url('/trips/')); ?>">🗺 Trips</a><a class="is-active" href="<?php echo esc_url(home_url('/saved/')); ?>">♡ Saved places</a><a href="<?php echo esc_url(home_url('/completed/')); ?>">🥾 Completed</a></nav>
            <section class="tng-trips-section">
                ...
                <?php if (!$logged_in): ?>
                    <div class="tng-trips-empty"><h3>Sign in to save places.</h3><p>Your saved trip will stay synced across phones, desktop, and TV games.</p><a class="tng-ui-button" href="<?php echo esc_url(wp_login_url(home_url('/saved/'))); ?>">Sign in</a></div>
                <?php elseif (!$posts): ?>
                    <div class="tng-trips-empty"><h3>Your trip is ready for its first stop.</h3><p>Explore the platform and add the places you want to visit.</p><a class="tng-ui-button" href="<?php echo esc_url(home_url('/explore/')); ?>">Find places</a></div>
                <?php else: ?>
                    <div class="tng-trip-suggestions">
                        <?php foreach ($posts as $post): $image = get_the_post_thumbnail_url($post->ID, 'medium_large'); $type = get_post_type_object(get_post_type($post->ID)); $label = $type && !empty($type->labels->singular_name) ? $type->labels->singular_name : 'Place'; ?>
                            <article class="tng-trip-suggestion" data-tng-saved-card="<?php echo esc_attr((string)$post->ID); ?>">
                                <a class="tng-trip-suggestion__media" href="<?php echo esc_url(get_permalink($post)); ?>"<?php echo $image ? ' style="background-image:url(' . esc_url($image) . ')"' : ''; ?>></a>
                                <div><small><?php echo esc_html($label); ?></small><h3><a href="<?php echo esc_url(get_permalink($post)); ?>"><?php echo esc_html(get_the_title($post)); ?></a></h3><button type="button" class="tng-trip-toggle is-saved" data-tng-trip-toggle data-post-id="<?php echo esc_attr((string)$post->ID); ?>">✓ Added to trip</button></div>
                            </article>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
