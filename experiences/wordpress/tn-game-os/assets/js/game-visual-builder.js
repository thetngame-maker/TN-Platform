(() => {
  ...
  const mount = () => {
      ...
      checkpoints.forEach((cp, index) => {
        ...
        row.innerHTML = `
          ...
          <div class="tng-visual-checkpoint__fields">
            ...
            ${cp.sightId ? `<small>📍 Linked Top Sight · coordinates filled automatically</small>` : (cp.type==='gps' ? `<small>📍 ${round(cp.lat)}, ${round(cp.lng)} · drag marker to adjust</small>` : '')}
          </div>
        ...
        row.querySelector('[data-remove]').addEventListener('click', () => { checkpoints.splice(index,1); sync(); render(); });
    ...
    const addGps = latlng => {
      checkpoints.push({ title:`${data.labels.checkpoint} ${checkpoints.length+1}`, instructions:'', type:'gps', lat:round(latlng.lat), lng:round(latlng.lng), radius:30 });
      render();
    };
    map.on('click', e => addGps(e.latlng));

    const parseGpx = text => {
      const xml = new DOMParser().parseFromString(text, 'application/xml');
      const nodes = [...xml.querySelectorAll('trkpt, rtept')];
      return nodes.map(n => [Number(n.getAttribute('lat')), Number(n.getAttribute('lon'))]).filter(p => Number.isFinite(p[0]) && Number.isFinite(p[1]));
    };

    const loadTrail = async () => {
      if (routeLayer) { map.removeLayer(routeLayer); routeLayer = null; }
      const trail = trailById.get(String(trailSelect.value));
      if (!trail) { routeStatus.textContent = 'No linked trail. Click the map to place checkpoints manually.'; return; }
      if (!trail.gpxUrl) { routeStatus.textContent = `${trail.title} is selected, but no GPX file was detected. You can still click the map to place checkpoints.`; return; }
      ...
      try {
        ...
        routeStatus.textContent = `${trail.title} route loaded. Click directly along the trail to add checkpoints.`;
      } catch (e) {
        routeStatus.textContent = `The trail is linked, but its GPX preview could not load. You can still place checkpoints manually.`;
      }
    };
    trailSelect.addEventListener('change', loadTrail);

    wrap.querySelector('[data-tng-add-sight]').addEventListener('click', () => {
      const sight = (data.sights || []).find(s => String(s.id) === String(sightSelect.value));
      if (!sight) return;
      checkpoints.push({ title:sight.title, instructions:`Visit ${sight.title}.`, type:'gps', sightId:sight.id, lat:Number(sight.lat), lng:Number(sight.lng), radius:30 });
      sync(); render();
