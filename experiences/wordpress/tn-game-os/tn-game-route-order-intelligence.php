 ...
 * Adds GPX mileage annotations, route-health warnings, and one-click checkpoint sorting.
 */
...
(()=>{
 ...
 const ensurePanel=()=>{const side=document.querySelector('.tng-route-editor__side');if(!side)return null;let p=side.querySelector('.tng-route-health');if(p)return p;p=document.createElement('section');p.className='tng-route-health';p.innerHTML='<div class="tng-route-health__head"><strong>Route health</strong><span class="tng-route-health__status">Analyzing</span></div><ul class="tng-route-health__issues"></ul><div class="tng-route-health__actions"><button type="button" class="button button-primary tng-route-sort">Sort by trail order</button><button type="button" class="button tng-route-health-refresh">Refresh</button></div>';const auto=side.querySelector('.tng-auto-route-builder');if(auto)auto.insertAdjacentElement('afterend',p);else side.prepend(p);p.querySelector('.tng-route-sort').onclick=sortRows;p.querySelector('.tng-route-health-refresh').onclick=render;return p;};
 ...
 const analyze=ms=>{const issues=[];let backwards=[];for(let i=1;i<ms.length;i++){if(ms[i-1].valid&&ms[i].valid&&ms[i].along+0.03<ms[i-1].along)backwards.push(i+1);}if(backwards.length)issues.push(`Checkpoint order moves backward on the GPX near stop${backwards.length>1?'s':''} ${backwards.join(', ')}.`);const invalid=ms.filter(m=>!m.valid);if(invalid.length)issues.push(`${invalid.length} checkpoint${invalid.length===1?'':'s'} ${invalid.length===1?'is':'are'} missing valid GPS coordinates.`);const far=ms.filter(m=>m.valid&&m.off>152.4);if(far.length)issues.push(`${far.length} checkpoint${far.length===1?' is':'s are'} more than 500 ft from the GPX route.`);let closePairs=[];for(let i=0;i<ms.length;i++)for(let j=i+1;j<ms.length;j++){if(!ms[i].valid||!ms[j].valid)continue;const d=hav([num(ms[i].row,'latitude'),num(ms[i].row,'longitude')],[num(ms[j].row,'latitude'),num(ms[j].row,'longitude')]);if(d<15.24)closePairs.push(`${i+1}/${j+1}`);}if(closePairs.length)issues.push(`Very close or duplicate checkpoints: ${closePairs.join(', ')}.`);return{issues,backwards};};
 function sortRows(){if(!routePts.length)return;const wrap=rowWrap();if(!wrap)return;const ms=metrics().filter(m=>m.valid).sort((a,b)=>a.along-b.along);const invalid=metrics().filter(m=>!m.valid);[...ms,...invalid].forEach(m=>wrap.appendChild(m.row));renumber();forceRebuild();setTimeout(render,80);}
 function render(){if(!routePts.length)return;const ms=metrics(),report=analyze(ms),panel=ensurePanel();if(!panel)return;const status=panel.querySelector('.tng-route-health__status'),list=panel.querySelector('.tng-route-health__issues'),sort=panel.querySelector('.tng-route-sort');status.textContent=report.issues.length?`${report.issues.length} issue${report.issues.length===1?'':'s'}`:'Route looks good';status.classList.toggle('is-warning',!!report.issues.length);list.innerHTML=report.issues.map(x=>`<li>${x}</li>`).join('');sort.disabled=ms.filter(m=>m.valid).length<2;annotateList(ms);const routeItems=[...(document.getElementById('tng-route-list')?.children||[])];routeItems.forEach(x=>x.classList.remove('is-order-warning'));report.backwards.forEach(n=>routeItems[n-1]?.classList.add('is-order-warning'));}
 const load=()=>{const route=(typeof TNG_ADMIN_ROUTE_EDITOR!=='undefined'&&TNG_ADMIN_ROUTE_EDITOR.routeUrl)||'';if(!route)return false;fetch(route,{credentials:'same-origin'}).then(r=>r.ok?r.text():Promise.reject()).then(text=>{const xml=new DOMParser().parseFromString(text,'application/xml');routePts=[...xml.querySelectorAll('trkpt,rtept')].map(n=>[parseFloat(n.getAttribute('lat')),parseFloat(n.getAttribute('lon'))]).filter(p=>valid(p[0],p[1]));buildCum();render();const rw=rowWrap();if(rw)new MutationObserver(()=>setTimeout(render,30)).observe(rw,{childList:true,subtree:true,attributes:false});const list=document.getElementById('tng-route-list');if(list)new MutationObserver(()=>setTimeout(render,30)).observe(list,{childList:true});rw?.addEventListener('change',()=>setTimeout(render,20));document.addEventListener('tng:checkpoint-form-changed',()=>setTimeout(render,20));}).catch(()=>{});return true;};
