...
final class TNG_Game_Game_Audit {
    ...
    private static function trail_has_route(int $trail_id): bool {
        ...
        foreach (['trail_gpx_url', '_trail_gpx_url', 'gpx_url', '_gpx_url'] as $key) {
            $value = trim((string) get_post_meta($trail_id, $key, true));
    ...
    private static function audit_game(WP_Post $game): array {
        ...
        if (!$checkpoints) {
            $errors[] = 'No structured checkpoints are saved.';
        } else {
            $passes[] = count($checkpoints) . ' structured checkpoint' . (count($checkpoints) === 1 ? '' : 's');
        }
        ...
        foreach ($checkpoints as $i => $cp) {
            ...
            if ($cp_type === 'gps') {
                if (!self::valid_coords($cp['latitude'] ?? null, $cp['longitude'] ?? null)) {
                    $errors[] = "Checkpoint {$n} GPS coordinates are missing or invalid.";
                }
        ...
        } elseif ($is_trail_game) {
            $errors[] = 'Trail game has no linked trail route source.';
        } else {
            $passes[] = 'Standalone checkpoint map allowed';
        }
        ...
        if ($last_pass !== '') $passes[] = 'Full guided test passed';
        else $warnings[] = 'No recorded full Guided Test Run pass yet.';

        $score = 100 - (count($errors) * 20) - (count($warnings) * 5);
        $score = max(0, min(100, $score));
        if ($errors) $status = 'Needs attention';
        elseif ($warnings) $status = 'Ready with warnings';
        else $status = 'Ready';

        return compact('errors','warnings','passes','score','status','checkpoints','calc_xp','saved_xp','last_pass');
    }
    ...
    public static function render(): void {
        ...
        $games = get_posts([
            ...
            'posts_per_page' => 300,
            'orderby' => ['post_status' => 'ASC', 'title' => 'ASC'],
        ]);
        $audits = [];
        $ready = $warning = $bad = 0;
        ...
        <div class="wrap tng-game-audit">
            ...
            <?php foreach ($games as $game): $a = $audits[$game->ID];
            ...
            ?>
                <section class="tng-audit-card">
                    ...
                    <div class="tng-audit-body">
                        ...
                        <div class="tng-audit-list is-pass"><h3>Passed · <?php echo esc_html((string) count($a['passes'])); ?></h3><ul><?php foreach ($a['passes'] as $item): ?><li><?php echo esc_html($item); ?></li><?php endforeach; ?></ul><?php if ($a['last_pass']): ?><div class="tng-audit-meta" style="margin-top:9px">Last guided pass: <?php echo esc_html($a['last_pass']); ?></div><?php endif; ?></div>
                    </div>
                </section>
            <?php endforeach; ?>
        </div>
