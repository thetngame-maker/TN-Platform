...
final class TNG_Game_Builder_UI {
    ...
    public static function render(): string {
        ...
        <section class="tng-game-builder-panel"><div class="tng-game-builder-heading"><span class="tng-eyebrow">Checkpoint plan</span><h2>Build the route</h2><p>Use one line per checkpoint. Stops can be verified directly or connected to an existing Top Sight.</p></div><div class="tng-game-builder-fields"><label class="is-wide"><span>Trail route source</span><select name="trail_route_source"><option value="0">No linked trail — checkpoint map only</option><?php foreach($route_sources as $source):?><option value="<?php echo esc_attr((string)$source->ID);?>"><?php echo esc_html(get_the_title($source));?></option><?php endforeach;?></select><small>Optional. Choose an existing trail Activity and the live game map will inherit its GPX route automatically.</small></label><label class="is-wide"><span>Checkpoints</span><textarea name="checkpoint_plan" rows="13" placeholder="Historic Depot | Find the old station sign. | tap&#10;Railroad Riddle | What year is shown on the marker? | question | 1902&#10;Sycamore Falls | Get close enough to unlock the stop. | gps | 35.2500 | -85.7500 | 30&#10;Mural Photo | Take a photo at the mural. | photo&#10;Foster Falls | Visit the Top Sight. | sight | 123 | gps | 30"></textarea><small><strong>Formats:</strong><br>Tap: Name | Instructions | tap<br>Question: Name | Question | question | Correct answer<br>GPS: Name | Instructions | gps | Latitude | Longitude | Radius meters<br>Photo: Name | Instructions | photo<br>Top Sight: Name | Instructions | sight | Top Sight post ID | gps/photo/tap | Radius meters</small></label><label><span>Fallback checkpoint count</span><input name="checkpoint_count" type="number" min="1" value="5"></label></div></section>
