...
final class TNG_Game_Builder_UI {
    ...
    public static function handle_submit(): void {
        ...
        if($plan!==''){foreach(preg_split('/\r\n|\r|\n/',$plan) as $line){$line=trim($line);if($line==='')continue;$parts=array_map('trim',explode('|',$line,2));$checkpoints[]=['title'=>sanitize_text_field($parts[0]),'instructions'=>sanitize_textarea_field($parts[1]??'')];}}
    ...
    public static function render(): string {
        ...
        <section class="tng-game-builder-panel"><div class="tng-game-builder-heading"><span class="tng-eyebrow">Checkpoint plan</span><h2>Build the route</h2><p>Enter one checkpoint per line. Add instructions after a vertical bar if you want.</p></div><div class="tng-game-builder-fields"><label class="is-wide"><span>Checkpoints</span><textarea name="checkpoint_plan" rows="8" placeholder="Historic Depot | Find the old station sign.&#10;Downtown Mural | Take a photo with the mural.&#10;Finish Line | Return to the starting point."></textarea><small>Format: Checkpoint name | Instructions</small></label><label><span>Fallback checkpoint count</span><input name="checkpoint_count" type="number" min="1" value="5"></label></div></section>
