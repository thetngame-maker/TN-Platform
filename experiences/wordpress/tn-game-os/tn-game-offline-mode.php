...
self.addEventListener('message',event=>{
  ...
  if(data.type==='TNG_ADVENTURE_PACK_REMOVE')event.waitUntil((async()=>{const id=adventureId(data.id);if(!id){reply({ok:false,error:'Unknown adventure pack.'});return;}await Promise.all([caches.delete(PACK_PREFIX+id),caches.delete(PACK_PREFIX+id+'-staging')]);reply({ok:true,id,installed:await adventureStatus([{id,urls:[]}])});})());
  if(data.type==='TNG_ADVENTURE_PACK_SAVE')event.waitUntil((async()=>{const id=adventureId(data.id);const values=Array.isArray(data.urls)?[...new Set(data.urls)].slice(0,12):[];if(!id||!values.length){reply({ok:false,error:'Adventure pack is not valid.'});return;}const liveName=PACK_PREFIX+id;const stagingName=liveName+'-staging';const previous=await cacheEntries(await caches.open(liveName));await caches.delete(stagingName);const staging=await caches.open(stagingName);let saved=0;const failed=[];for(const value of values){const url=adventureUrl(value);if(!url){failed.push(value);continue;}try{const response=await fetch(new Request(url,{credentials:'omit',cache:'reload',redirect:'follow'}));if(response.ok&&response.headers.get('X-TNG-Offline-Safe')==='1'){await staging.put(url,response.clone());saved++;}else failed.push(url);}catch(error){failed.push(url);}}if(failed.length||saved!==values.length){await caches.delete(stagingName);reply({ok:false,id,saved:0,failed:failed.length||values.length-saved,preserved:previous.length,installed:await adventureStatus([{id,urls:values}])});return;}const fresh=await cacheEntries(staging);try{await caches.delete(liveName);await writeEntries(liveName,fresh);await caches.delete(stagingName);reply({ok:true,id,saved:fresh.length,failed:0,preserved:0,installed:await adventureStatus([{id,urls:values}])});}catch(error){await caches.delete(liveName);await writeEntries(liveName,previous);await caches.delete(stagingName);reply({ok:false,id,saved:0,failed:values.length,preserved:previous.length,installed:await adventureStatus([{id,urls:values}])});}})());
});
        <?php
        exit;
    }
