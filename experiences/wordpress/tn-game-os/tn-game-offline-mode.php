...
self.addEventListener('message',event=>{
  ...
  const adventureStatus=async packs=>{const installed={};for(const value of packs.slice(0,12)){const entry=typeof value==='string'?{id:value,urls:[]}:value||{};const id=adventureId(entry.id);if(!id)continue;const expected=[...new Set((Array.isArray(entry.urls)?entry.urls:[]).map(adventureUrl).filter(Boolean))].slice(0,12);const cache=await caches.open(ADVENTURE_PACK_PREFIX+id);const saved=(await cache.keys()).map(request=>request.url);const metadata=await adventureMeta(id);const verifiedAt=typeof metadata.verifiedAt==='string'?metadata.verifiedAt:'';const verifiedTime=Date.parse(verifiedAt);installed[id]={count:saved.length,current:expected.length?expected.length===saved.length&&expected.every(url=>saved.includes(url)):saved.length===0,verifiedAt,stale:saved.length>0&&(!Number.isFinite(verifiedTime)||Date.now()-verifiedTime>604800000)};}return installed;};
  ...
  const adventureLibrary=async()=>{const packs=[];for(const key of await caches.keys()){if(packs.length>=12)break;if(!key.startsWith(ADVENTURE_PACK_PREFIX)||key.endsWith('-staging'))continue;const id=adventureId(key.slice(ADVENTURE_PACK_PREFIX.length));if(!id)continue;const cache=await caches.open(key);const requests=await cache.keys();if(!requests.length)continue;const urls=[];let bytes=0;let issues=0;for(const request of requests){const url=adventureUrl(request.url);if(!url){issues++;continue;}try{const response=await cache.match(request);const type=response?response.headers.get('Content-Type')||'':'';if(!response||!response.ok||response.headers.get('X-TNG-Offline-Safe')!=='1'||!type.toLowerCase().includes('text/html')){issues++;continue;}const size=(await response.clone().blob()).size;if(!size){issues++;continue;}urls.push(url);bytes+=size;}catch(error){issues++;}}const metadata=await adventureMeta(id);packs.push({id,count:requests.length,healthyCount:urls.length,issues,urls,bytes,verifiedAt:typeof metadata.verifiedAt==='string'?metadata.verifiedAt:''});}return packs;};
  const clearAdventureLibrary=async()=>{const keys=(await caches.keys()).filter(key=>key.startsWith(ADVENTURE_PACK_PREFIX));const removed=keys.filter(key=>!key.endsWith('-staging')).length;await Promise.all([...keys.map(key=>caches.delete(key)),caches.delete(ADVENTURE_META_CACHE)]);return removed;};
  ...
  if(data.type==='TNG_ADVENTURE_PACK_LIBRARY')event.waitUntil(adventureLibrary().then(packs=>reply({ok:true,packs})));
  if(data.type==='TNG_ADVENTURE_PACK_CLEAR')event.waitUntil(clearAdventureLibrary().then(removed=>reply({ok:true,removed,packs:[]})));
  ...
  if(data.type==='TNG_ADVENTURE_PACK_SAVE')event.waitUntil((async()=>{const id=adventureId(data.id);const values=Array.isArray(data.urls)?[...new Set(data.urls)].slice(0,12):[];if(!id||!values.length){reply({ok:false,error:'Adventure pack is not valid.'});return;}const liveName=ADVENTURE_PACK_PREFIX+id;const stagingName=liveName+'-staging';const previous=await cacheEntries(await caches.open(liveName));const storage=await storageHeadroom();if(!storage.ok){reply({ok:false,error:'low-storage',id,saved:0,failed:values.length,preserved:previous.length,storage,installed:await adventureStatus([{id,urls:values}])});return;}await caches.delete(stagingName);const staging=await caches.open(stagingName);let saved=0;const failed=[];for(const value of values){const url=adventureUrl(value);if(!url){failed.push(value);continue;}try{const response=await fetch(new Request(url,{credentials:'omit',cache:'reload',redirect:'follow'}));if(response.ok&&response.headers.get('X-TNG-Offline-Safe')==='1'){await staging.put(url,response.clone());saved++;}else failed.push(url);}catch(error){failed.push(url);}}if(failed.length||saved!==values.length){await caches.delete(stagingName);reply({ok:false,id,saved:0,failed:failed.length||values.length-saved,preserved:previous.length,installed:await adventureStatus([{id,urls:values}])});return;}const fresh=await cacheEntries(staging);try{await caches.delete(liveName);await writeEntries(liveName,fresh);await caches.delete(stagingName);try{await setAdventureMeta(id,{verifiedAt:new Date().toISOString(),count:fresh.length});}catch(error){}reply({ok:true,id,saved:fresh.length,failed:0,preserved:0,installed:await adventureStatus([{id,urls:values}])});}catch(error){await caches.delete(liveName);await writeEntries(liveName,previous);await caches.delete(stagingName);reply({ok:false,id,saved:0,failed:values.length,preserved:previous.length,installed:await adventureStatus([{id,urls:values}])});}})());
    ...
    public static function render_screen(): string {
        ...
        <main class="tng-offline-screen tng-app-shell" data-tng-offline-manager>
            ...
            <section class="tng-offline-adventures" data-tng-device-adventures hidden><header><div><span class="tng-eyebrow">Device library</span><h2>Saved Adventures on this device</h2></div><div class="tng-offline-adventures__tools"><small data-tng-device-adventure-summary>Public stop screens only · plan details stay private</small><button type="button" data-tng-device-adventure-clear hidden>Remove all from device</button></div></header><div class="tng-offline-adventures__list" data-tng-device-adventure-list></div></section>
