...
final class Gameplay_Event_Service implements Module_Interface {
    ...
    public function count_user_events(int $user_id, string $event_type, string $since): int {
        $this->ensure_table(); global $wpdb;
        return (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$this->table()} WHERE user_id=%d AND event_type=%s AND occurred_at >= %s", $user_id, sanitize_key($event_type), $this->mysql_date($since)));
    }

    public function community_totals(string $since): array {
    ...
    public function menu(): void { add_submenu_page('tn-game-os', 'Gameplay Event Ledger', 'Gameplay Events', 'manage_options', self::PAGE, [$this, 'page']); }

    public function page(): void {
        if (!current_user_can('manage_options')) wp_die('Unauthorized.');
        $this->ensure_table(); global $wpdb; $table = $this->table();
        ...
        <?php foreach ([['All events',$total],['Events today',$today],['Explorers recorded',$users]] as [$label,$value]): ?><div style="background:#fff;border:1px solid #dcdcde;border-radius:14px;padding:18px"><strong style="display:block;font-size:30px;color:#6438b3"><?php echo number_format_i18n($value); ?></strong><span><?php echo esc_html($label); ?></span></div><?php endforeach; ?></div>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('Rebuild the canonical event ledger from all current Explorer profiles? Existing event keys will not be duplicated.');" style="margin-bottom:20px"><?php wp_nonce_field('tng_rebuild_gameplay_events'); ?><input type="hidden" name="action" value="tng_rebuild_gameplay_events"><button class="button button-secondary">Rebuild from profiles</button></form>
    ...
    public function rebuild(): void {
        if (!current_user_can('manage_options')) wp_die('Unauthorized.'); check_admin_referer('tng_rebuild_gameplay_events'); $this->ensure_table();
        foreach (get_users(['fields' => 'ids', 'meta_key' => '_tng_explorer_profile']) as $user_id) {
            $profile = get_user_meta((int)$user_id, '_tng_explorer_profile', true); if (!is_array($profile)) continue;
            foreach ((array)($profile['recentActivity'] ?? []) as $activity) {
                if (!is_array($activity)) continue; $kind = sanitize_key((string)($activity['kind'] ?? 'checkpoint')); $id = sanitize_text_field((string)($activity['id'] ?? '')); if ($id === '') continue;
        ...
        wp_safe_redirect(admin_url('admin.php?page=' . self::PAGE . '&rebuilt=1')); exit;
    }
