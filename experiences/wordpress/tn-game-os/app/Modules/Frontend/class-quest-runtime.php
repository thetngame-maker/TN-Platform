    ...
    private function render(int $quest_id): string {
        ...
        <section class="tng-runtime" data-quest-id="<?php echo esc_attr((string)$quest_id); ?>">
            <style>
                ...
                .tng-runtime-sync{font-size:12px;margin-top:10px;color:rgba(255,255,255,.78)}.tng-runtime-panel{background:#fff;border:1px solid #e4e7ec;border-radius:18px;padding:20px;margin-top:16px}.tng-runtime-progress{height:11px;background:#eceff3;border-radius:999px;overflow:hidden}.tng-runtime-progress span{display:block;height:100%;width:0;background:#12b76a;transition:.25s}
            ...
            <div class="tng-runtime-panel" hidden>
                ...
                <div class="tng-runtime-complete"><h3>Quest complete!</h3><p>You completed the required checkpoints and earned <span data-final-xp>0</span> checkpoint XP.</p></div>
            </div>
            <script type="application/json" class="tng-runtime-data"><?php echo wp_json_encode($payload); ?></script>
            <script>
            (()=>{const root=document.currentScript.closest('.tng-runtime');if(!root)return;const data=JSON.parse(root.querySelector('.tng-runtime-data').textContent),panel=root.querySelector('.tng-runtime-panel'),list=root.querySelector('.tng-runtime-list'),bar=root.querySelector('.tng-runtime-progress span'),error=root.querySelector('.tng-runtime-error'),storage='tngQuestProgress:'+data.questId;let state={started:false,done:[],status:'not_started'},saving=false;const escapeHtml=v=>String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));const localLoad=()=>{try{return JSON.parse(localStorage.getItem(storage)||'{}')}catch(e){return{}}};const localSave=()=>{try{localStorage.setItem(storage,JSON.stringify(state))}catch(e){}};const showError=message=>{error.textContent=message||'';error.classList.toggle('is-visible',Boolean(message));};const api=async(method,body)=>{const response=await fetch(data.progressUrl,{method,credentials:'same-origin',headers:{'Content-Type':'application/json','X-WP-Nonce':data.restNonce},body:body?JSON.stringify(body):undefined});if(!response.ok)throw new Error('Progress could not be synced.');return response.json();};const apply=remote=>{state.started=Boolean(remote.started);state.done=Array.isArray(remote.completedStops)?remote.completedStops.map(String):[];state.status=remote.status||'not_started';};const persist=async()=>{localSave();if(!data.loggedIn||saving)return;saving=true;showError('');try{const remote=await api('POST',{started:state.started,completedStops:state.done});apply(remote);localSave();}catch(e){showError('Your progress is saved on this device, but account sync is temporarily unavailable.');}finally{saving=false;render();}};const render=()=>{panel.hidden=!state.started;root.querySelector('.tng-runtime-start').textContent=state.started?'Resume Quest':'Start Quest';const done=new Set((state.done||[]).map(String));let earned=0;data.stops.forEach(s=>{if(done.has(String(s.id)))earned+=Number(s.xp||0)});root.querySelector('[data-complete]').textContent=done.size;root.querySelector('[data-earned]').textContent=earned;root.querySelector('[data-final-xp]').textContent=earned;bar.style.width=(data.required?Math.min(100,done.size/data.required*100):0)+'%';list.innerHTML=data.stops.map((s,i)=>{const id=String(s.id),complete=done.has(id),unlocked=state.started&&(i===0||done.has(String(data.stops[i-1].id))||complete);return `<article class="tng-runtime-stop ${complete?'is-done':''} ${unlocked?'':'is-locked'}"><span class="tng-runtime-num">${complete?'✓':i+1}</span><div><h3>${escapeHtml(s.title)}</h3><p>${escapeHtml(s.instruction||s.arrival||'Reach this checkpoint to continue.')}</p><small>${escapeHtml(s.type)} · ${Number(s.xp||0)} XP</small></div><button type="button" class="tng-runtime-claim" data-id="${escapeHtml(id)}" ${unlocked||complete?'':'disabled'}>${complete?'Claimed':'Claim checkpoint'}</button></article>`}).join('');root.querySelector('.tng-runtime-complete').classList.toggle('is-visible',done.size>=data.required&&data.required>0);};root.querySelector('.tng-runtime-start').addEventListener('click',()=>{state.started=true;persist();render();panel.scrollIntoView({behavior:'smooth',block:'start'});});list.addEventListener('click',e=>{const button=e.target.closest('[data-id]');if(!button||button.disabled)return;const id=String(button.dataset.id);state.done=Array.from(new Set([...(state.done||[]).map(String),id]));persist();render();});const init=async()=>{apply(Object.assign({started:false,completedStops:[]},localLoad(),{completedStops:localLoad().done||localLoad().completedStops||[]}));if(data.loggedIn){try{const remote=await api('GET');apply(remote);localSave();}catch(e){showError('Account progress could not be loaded. Using progress saved on this device.');}}render();};init();})();
            </script>
        </section>
