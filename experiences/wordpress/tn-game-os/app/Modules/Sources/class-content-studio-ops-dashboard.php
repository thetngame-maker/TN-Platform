...
final class Content_Studio_Ops_Dashboard implements Module_Interface {
    ...
    private function monitor_settings(): array {
        $value = get_option(self::MONITOR_OPTION, []);
        return is_array($value) ? $value : [];
    }

    private function monitor_log(): array {
        $value = get_option(self::MONITOR_LOG_OPTION, []);
        return is_array($value) ? $value : [];
    }
    ...
    private function actionable_counts(array $history): array {
        ...
        foreach ($history as $town_history) {
            ...
            foreach ($snapshot as $item) {
                ...
                if (!empty($item['_review_status'])) continue;
    ...
    private function candidate_count(): int {
        $counts = wp_count_posts(self::CANDIDATE_CPT);
        if (!$counts) return 0;
        return absint($counts->publish ?? 0) + absint($counts->draft ?? 0) + absint($counts->pending ?? 0) + absint($counts->private ?? 0);
    }

    private function towns_count(array $history): int {
        return count(array_filter($history, 'is_array'));
    }
    ...
    private function card(string $label, string $value, string $detail='', string $accent='#2271b1'): void {
        ?>
        <div style="background:#fff;border:1px solid #dcdcde;border-left:4px solid <?php echo esc_attr($accent); ?>;border-radius:8px;padding:16px;min-width:180px;flex:1">
    ...
    public function render_page(): void {
        ...
        $settings = $this->monitor_settings();
        $log = $this->monitor_log();
        $actions = $this->actionable_counts($history);
        $scans = $this->scan_stats($history);
        $failures = $this->failures_24h($log);
        $next = wp_next_scheduled(self::CRON_HOOK);
        $enabled = !empty($settings['enabled']);
        $wp_cron_disabled = defined('DISABLE_WP_CRON') && DISABLE_WP_CRON;
        $monitor_towns = $this->monitor_towns($settings);
        $candidate_count = $this->candidate_count();
        $last_log = !empty($log[0]) && is_array($log[0]) ? $log[0] : [];

        if (!$enabled) {
            $cron_label = 'Off'; $cron_detail = 'Automatic monitoring disabled'; $cron_color = '#646970';
        } elseif (!$next) {
            $cron_label = 'Needs attention'; $cron_detail = 'Monitoring enabled but no event is scheduled'; $cron_color = '#b32d2e';
        } elseif ($wp_cron_disabled) {
            $cron_label = 'Server cron needed'; $cron_detail = 'WP-Cron trigger is disabled'; $cron_color = '#b26200';
        } else {
            $cron_label = 'Scheduled'; $cron_detail = 'Next ' . wp_date('M j, g:i A', $next); $cron_color = '#008a20';
        }
        ?>
        <div class="wrap">
            <h1>📊 Content Studio Operations</h1>
            <p>Health and activity across Local Discovery, Town Scanner, Changes Inbox, and Town Monitoring.</p>

            <div style="display:flex;gap:12px;flex-wrap:wrap;margin:20px 0">
                <?php $this->card('Monitor health',$cron_label,$cron_detail,$cron_color); ?>
                ...
                <div style="background:#fff;border:1px solid #dcdcde;border-radius:10px;padding:18px">
                    ...
                    <table class="widefat striped">
                        <tbody>
                            ...
                            <tr><td>Next scheduled event</td><td><?php echo $next ? esc_html(wp_date('Y-m-d g:i A',$next)) : 'Not scheduled'; ?></td></tr>
                            <tr><td>Cadence</td><td><?php echo esc_html(ucfirst((string)($settings['cadence'] ?? 'weekly'))); ?></td></tr>
                            <tr><td>Maximum results / town</td><td><?php echo absint($settings['max_items'] ?? 50); ?></td></tr>
                        </tbody>
                    </table>
                    <?php if ($wp_cron_disabled): ?>
                        <div class="notice notice-warning inline" style="margin:14px 0 0"><p><strong>WP-Cron is disabled.</strong> Configure a real server cron to request <code>wp-cron.php</code> regularly so scheduled monitoring runs reliably.</p></div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <?php if (!$log): ?>
                <p>No monitoring runs have been recorded yet.</p>
            <?php else: ?>
                <table class="widefat striped" style="max-width:1100px">
                    <thead><tr><th>Time</th><th>Town</th><th>Run</th><th>Result</th></tr></thead>
                    <tbody>
            ...
            <?php if ($failures): ?>
                <h2 style="margin-top:26px;color:#b32d2e">Failures in the last 24 hours</h2>
                <table class="widefat striped" style="max-width:1100px">
                    <thead><tr><th>Time</th><th>Town</th><th>Error</th></tr></thead><tbody>
                    <?php foreach ($failures as $row): ?><tr><td><?php echo esc_html((string)($row['time'] ?? '')); ?></td><td><?php echo esc_html((string)($row['town'] ?? '')); ?></td><td><?php echo esc_html((string)($row['message'] ?? '')); ?></td></tr><?php endforeach; ?>
                    </tbody>
                </table>
            <?php endif; ?>
        </div>
