...
final class Launch_Readiness implements Module_Interface {
    ...
    private const DEMO_GEO_TERMS = [
        'California', 'Los Angeles', 'San Francisco', 'San Diego', 'New York',
        'Las Vegas', 'Nevada', 'Miami', 'Florida', 'Chicago', 'Illinois',
        'London', 'Paris', 'Dubai', 'Rome', 'Barcelona', 'Tokyo',
    ];

    private const LEGACY_BRAND_TERMS = [
        'Traveler', 'travelerwp', 'COVID-19 Response', 'Cancellation options',
        'Help Center', '© Copyright Traveler', 'Copyright Traveler 2022',
    ];

    private const PLACEHOLDER_TERMS = [
        'Lorem ipsum', 'Sample Page', 'Demo', 'Dummy', 'Test Listing',
    ];
    ...
    public function render(): void {
        ...
        <div class="wrap">
            ...
            <?php if (isset($_GET['rescanned'])): ?>
                <div class="notice notice-success is-dismissible"><p>Launch-readiness audit refreshed.</p></div>
            <?php endif; ?>
            ...
            <div style="display:grid;grid-template-columns:minmax(250px,1.2fr) repeat(4,minmax(150px,1fr));gap:14px;max-width:1250px;margin:22px 0;align-items:stretch;">
                ...
                <?php
                $cards = [
                    ['High priority', (int)($counts['high'] ?? 0)],
                    ['Medium', (int)($counts['medium'] ?? 0)],
                    ['Low', (int)($counts['low'] ?? 0)],
                    ['Public items scanned', (int)($report['scanned'] ?? 0)],
                ];
            ...
            <div style="background:#fff;border:1px solid #dcdcde;border-radius:14px;overflow:hidden;max-width:1250px;">
                <div style="padding:20px 22px;border-bottom:1px solid #eee;">
                    <h2 style="margin:0;">Launch blockers & recommendations</h2>
                    <p style="margin:6px 0 0;color:#646970;">Review high-priority items first. Nothing here is removed or unpublished automatically.</p>
                </div>
                <?php if (!$issues): ?>
                ...
                <?php else: ?>
                    <table class="widefat striped" style="border:0;box-shadow:none;">
                        ...
                        <?php foreach ($issues as $issue):
                            ...
                            $badge_color = $priority === 'high' ? '#b42318' : ($priority === 'low' ? '#475467' : '#8a6400');
                            ?>
                            <tr>
                                <td><span style="display:inline-block;padding:4px 8px;border-radius:999px;background:<?php echo esc_attr($badge_color); ?>;color:#fff;font-size:11px;font-weight:700;text-transform:uppercase;"><?php echo esc_html($priority); ?></span></td>
            ...
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;max-width:1250px;margin-top:18px;">
                <?php
                $next = [
                    ['1. Clean public inventory', 'Remove or unpublish demo/foreign Traveler listings and obsolete footer/menu content before launch.'],
                    ['2. Verify mobile shell', 'Test Explore, Map, Play, Trips, and Profile at narrow mobile widths after cleanup.'],
                    ['3. Turn on intelligence', 'Once the public data layer is clean, Adventure AI and recommendation systems can safely use it as their source of truth.'],
                ];
                foreach ($next as [$title, $text]): ?>
    ...
    private function scan(bool $force): array {
        ...
        foreach ($ids as $post_id) {
            ...
            foreach (self::LEGACY_BRAND_TERMS as $term) {
                if ($term !== '' && strpos($haystack, strtolower($term)) !== false) {
                    $issues[] = $this->issue('high', 'Traveler/demo branding found', sprintf('%s contains “%s”.', $post->post_title ?: ('Post #' . $post_id), $term), 'Brand cleanup', get_edit_post_link($post_id, ''));
                    break;
                }
            }

            if ($is_listing) {
                foreach (self::DEMO_GEO_TERMS as $term) {
                    if (strpos($haystack, strtolower($term)) !== false) {
                        $issues[] = $this->issue('high', 'Possible non-Tennessee demo inventory', sprintf('%s references %s.', $post->post_title ?: ('Listing #' . $post_id), $term), 'Inventory', get_edit_post_link($post_id, ''));
                        break;
                    }
                }
            }
            ...
            foreach (self::PLACEHOLDER_TERMS as $term) {
                if (strpos($haystack, strtolower($term)) !== false) {
                    $issues[] = $this->issue('medium', 'Placeholder/test content detected', sprintf('%s contains “%s”.', $post->post_title ?: ('Post #' . $post_id), $term), 'Content quality', get_edit_post_link($post_id, ''));
                    break;
                }
            }

            if ($post->post_status === 'publish' && trim(wp_strip_all_tags($post->post_content)) === '' && in_array($post->post_type, ['page', 'post', 'st_activity', 'st_hotel', 'st_tours', 'st_rental'], true)) {
                $issues[] = $this->issue('low', 'Published item has no body content', $post->post_title ?: ('Post #' . $post_id), 'Content quality', get_edit_post_link($post_id, ''));
            }
        }
        ...
        foreach ($menus as $menu) {
            ...
            foreach ($items as $item) {
                $label = strtolower((string)$item->title . ' ' . (string)$item->url);
                foreach (self::LEGACY_BRAND_TERMS as $term) {
                    if (strpos($label, strtolower($term)) !== false) {
                        $issues[] = $this->issue('high', 'Legacy Traveler navigation link', sprintf('%s → %s', $menu->name, $item->title), 'Navigation', admin_url('nav-menus.php?action=edit&menu=' . (int)$menu->term_id));
                        break;
                    }
                }
            }
        ...
        $widget_options = ['widget_text', 'widget_custom_html', 'widget_nav_menu'];
        foreach ($widget_options as $option_name) {
            $value = get_option($option_name, []);
            if (!is_array($value)) continue;
            $serialized = strtolower(wp_json_encode($value));
            foreach (self::LEGACY_BRAND_TERMS as $term) {
                if (strpos($serialized, strtolower($term)) !== false) {
                    $issues[] = $this->issue('high', 'Legacy Traveler footer/widget content', sprintf('%s contains “%s”.', $option_name, $term), 'Footer / widgets', admin_url('widgets.php'));
                    break;
                }
            }
        }
        ...
        foreach ($destination_ids as $destination_id) {
            ...
            $related = 0;
            $meta = get_post_meta($destination_id);
            foreach ($meta as $key => $values) {
                if (strpos((string)$key, 'count') === false && strpos((string)$key, 'related') === false && strpos((string)$key, 'relationship') === false) continue;
                foreach ((array)$values as $raw) {
                    $maybe = maybe_unserialize($raw);
                    if (is_numeric($maybe)) $related = max($related, (int)$maybe);
                    if (is_array($maybe)) $related = max($related, count($maybe));
                }
            }
            if ($related === 0) {
                $issues[] = $this->issue('medium', 'Destination may have no connected experiences', $title ?: ('Destination #' . $destination_id), 'Destinations', get_edit_post_link($destination_id, ''));
            }
        }
        ...
        foreach ($issues as $issue) {
            ...
            if (isset($counts[$priority])) $counts[$priority]++;
        }

        $score = 100;
        $score -= min(60, $counts['high'] * 10);
        $score -= min(30, $counts['medium'] * 4);
        $score -= min(10, $counts['low']);
    ...
    private function issue(string $priority, string $title, string $detail, string $area, string $url = ''): array {
        return compact('priority', 'title', 'detail', 'area', 'url');
    }
    ...
    private function dedupe(array $issues): array {
        ...
        $seen = [];
        foreach ($issues as $issue) {
            $key = md5(wp_json_encode([$issue['priority'] ?? '', $issue['title'] ?? '', $issue['detail'] ?? '', $issue['area'] ?? '']));
