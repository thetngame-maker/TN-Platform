...
final class App_Router {
    ...
    public static function resolve_route(): void {
        ...
        if(!$route){$path=trim((string)wp_parse_url($_SERVER['REQUEST_URI']??'/',PHP_URL_PATH),'/');$candidate=explode('/',$path)[0]??'';if(in_array($candidate,self::ROUTES,true))$route=$candidate;}
        if(!in_array($route,self::ROUTES,true))return; self::$route=$route; global $wp_query; $wp_query->is_404=false;$wp_query->is_page=true;$wp_query->is_search=false;status_header(200);
    }
    ...
    public static function document_title(array $parts): array {
        ...
        $titles=['explore'=>'Explore','play'=>'Play','map'=>'Map','trips'=>'Trips','saved'=>'Saved Places','profile'=>'Explorer Profile','profile-settings'=>'Profile Settings','search'=>'Search','leaderboard'=>'Explorer Leaderboard','achievements'=>'Achievements','friends'=>'Friends','activity'=>'Explorer Activity','challenges'=>'Challenges','journal'=>'Explorer Journal','explorer-journal'=>'Explorer Journal','completed'=>'Completed Adventures','my-photos'=>'My Photos','trails'=>'Trails','events'=>'Events','food'=>'Food and Drink','top-sights'=>'Top Sights','destinations'=>'Destinations'];
        $parts['title']=$titles[self::$route]??'The TN Game';return $parts;
    }
    ...
    private static function screen(string $eyebrow,string $title,string $copy,array $actions): string{ob_start();?><main class="tng-native-screen tng-app-shell"><section class="tng-native-hero"><span class="tng-eyebrow"><?php echo esc_html($eyebrow);?></span><h1><?php echo esc_html($title);?></h1><p><?php echo esc_html($copy);?></p></section></main><?php return(string)ob_get_clean();}
}
App_Router::boot();
