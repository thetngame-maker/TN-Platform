...
final class TNG_Map_UI {
    ...
    private static function coords_from_value($value): array {
        ...
        if (is_string($value) && preg_match('/(-?\d{1,2}\.\d+)\s*[,| ]\s*(-?\d{1,3}\.\d+)/', $value, $m)) {
            if (self::valid_coords($m[1], $m[2])) return [(float) $m[1], (float) $m[2]];
        }
        return [];
    }
    ...
    private static function kind(int $id): string {
        ...
        if ($type === 'tng_game' || ($type === 'game')) return 'game';
        if (in_array($type, ['top_sight','top-sights','top_sights'], true)) return 'sight';
        ...
        if (class_exists('TNG_Event_UI') && TNG_Event_UI::is_event($id)) return 'event';
        if (class_exists('TNG_Trail_UI') && TNG_Trail_UI::is_trail($id)) return 'trail';
        $text = strtolower(get_the_title($id) . ' ' . get_post_field('post_content', $id));
        if (preg_match('/restaurant|food|cafe|coffee|burger|kitchen|grill|dining/', $text)) return 'food';
        if (preg_match('/trail|hike|hiking|loop|overlook/', $text)) return 'trail';
        return 'place';
    }

    private static function label(string $kind): string {
        return ['trail'=>'Trail','game'=>'Game','sight'=>'Top Sight','food'=>'Food','event'=>'Event','destination'=>'Destination','place'=>'Place'][$kind] ?? 'Place';
    }
    ...
    private static function excerpt(int $id): string {
        ...
        return wp_trim_words($source, 12, '…');
    }

    private static function candidate_posts(): array {
        $types = array_values(array_filter(['tng_game','game','st_activity','activity','top_sight','top-sights','top_sights','tng_destination','st_location'], 'post_type_exists'));
        if (!$types) return [];
        $query = new WP_Query([
            'post_type' => $types,
            'post_status' => 'publish',
    ...
    private static function items(): array {
        $items = [];
        foreach (self::candidate_posts() as $post) {
            $id = (int) $post->ID;
            if (get_post_type($id) === 'tng_game' && class_exists('TNG_Games_UI') && !TNG_Games_UI::is_player_ready($id)) continue;
    ...
    public static function assets(): void {
        ...
        wp_enqueue_script('tng-map-ui-live', TNG_OS_URL . 'assets/js/map-ui.js', ['tng-leaflet'], '0.3.0', true);
        $center = [35.2600, -85.7500];
        if ($items) $center = [(float) $items[0]['lat'], (float) $items[0]['lng']];
        wp_localize_script('tng-map-ui-live', 'TNG_DISCOVERY_MAP', ['items'=>$items,'center'=>$center,'zoom'=>10]);
    }
    ...
    private static function cards(array $items): string {
        ...
        foreach (array_slice($items, 0, 24) as $item) {
            echo '<article class="tng-map-result" data-tng-map-result="' . esc_attr((string) $item['id']) . '" data-kind="' . esc_attr($item['kind']) . '">';
            echo '<span class="tng-map-result__media"' . ($item['image'] ? ' style="background-image:url(' . esc_url($item['image']) . ')"' : '') . '></span>';
            echo '<span class="tng-map-result__copy"><small>' . esc_html($item['label']) . '</small><strong>' . esc_html($item['title']) . '</strong>';
            if ($item['subtitle']) echo '<em>' . esc_html($item['subtitle']) . '</em>';
            echo '<a data-tng-open-details href="' . esc_url($item['url']) . '">View details →</a></span></article>';
        }
        echo '</div>';
    ...
    public static function render(): string {
        ...
        $counts = array_fill_keys(['trail','game','sight','food'], 0);
        foreach ($items as $item) if (isset($counts[$item['kind']])) $counts[$item['kind']]++;
        ob_start(); ?>
        <main class="tng-map-screen tng-app-shell">
            <section class="tng-map-toolbar">
                <div><span class="tng-eyebrow">Explore nearby</span><h1>Adventure map</h1><p>See trails, games, sights, food, and local places together on one live Tennessee map.</p></div>
            ...
            <section class="tng-map-filterbar" aria-label="Map filters">
                <button class="is-active" data-tng-map-filter="all" type="button">All <span><?php echo count($items); ?></span></button>
            ...
            <section class="tng-map-layout">
                <div class="tng-map-canvas-wrap">
                    <div id="tng-discovery-map" class="tng-map-canvas" aria-label="Interactive Tennessee discovery map"></div>
                    <div class="tng-map-live-status"><span class="tng-map-live-dot"></span><strong>Live discovery map</strong><small data-tng-map-count><?php echo count($items); ?> places on map</small></div>
                </div>

                <aside class="tng-map-panel">
                    <div class="tng-map-panel__heading"><div><span class="tng-eyebrow">Around Tennessee</span><h2>Discoveries</h2></div><a href="<?php echo esc_url(home_url('/search/')); ?>">Search all</a></div>
                    <p class="tng-map-panel__intro">Tap a card to find it on the map, or tap a marker to preview it here.</p>
                    <?php echo $cards; ?>
                </aside>
            </section>
        </main>
