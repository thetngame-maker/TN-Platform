 ...
 * Reliable current objective, map focus, directions, proximity and adventure progress.
 */
...
final class TNG_Active_Mission_Navigator {
    ...
    public static function enqueue(): void {
        ...
        .tng-active-mission{position:relative;display:grid;grid-template-columns:auto minmax(0,1fr);gap:11px 12px;width:min(100%,690px);margin:12px 0 14px;padding:13px 14px 16px;border:1px solid #dfe8e2;border-radius:16px;background:#fff;box-shadow:0 8px 24px rgba(19,54,38,.06);color:#173b2a;overflow:hidden;transition:border-color .2s ease,box-shadow .2s ease,background .2s ease}
        .tng-active-mission__step{display:grid;place-items:center;width:42px;height:42px;border-radius:13px;background:#f26722;color:#fff;font-weight:900;font-size:15px;box-shadow:0 5px 14px rgba(242,103,34,.22);transition:transform .2s ease,box-shadow .2s ease}
...
(()=>{
 ...
 let livePos=null,lastArrivalKey='';
 ...
 const render=()=>{
   ...
   if(arrived){state='arrived';statusLabel='Arrived';statusClass=' is-ready';distanceClass=' is-arrived';announceArrival(String(n)+':'+String(cp.title||''));}
 ...
 const start=()=>{startGPS();render();new MutationObserver(queue).observe(document.body,{childList:true,subtree:true,characterData:true});setInterval(render,2000);};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();
})();
JS
        ,'after');
    }
