...
add_action('wp_enqueue_scripts', static function (): void {
    ...
    wp_enqueue_style('tng-platform-ui', TNG_OS_URL . 'assets/css/platform-ui.css', [], '2.1.0');
    wp_enqueue_style('tng-platform-ui-refinements', TNG_OS_URL . 'assets/css/platform-ui-refinements.css', ['tng-platform-ui'], '2.1.0');
    wp_enqueue_style('tng-app-router', TNG_OS_URL . 'assets/css/app-router.css', ['tng-platform-ui'], '2.8.0');
    wp_enqueue_style('tng-ui-kit', TNG_OS_URL . 'assets/css/ui-kit.css', ['tng-platform-ui', 'tng-app-router'], '2.6.0');
    $route = \TNG_OS\Platform\App_Router::current_route();
    if ($route === 'play') wp_enqueue_style('tng-play-ui', TNG_OS_URL . 'assets/css/play-ui.css', ['tng-ui-kit'], '0.3.3');
    if ($route === 'map') wp_enqueue_style('tng-map-ui', TNG_OS_URL . 'assets/css/map-ui.css', ['tng-ui-kit'], '0.3.3');
    if (in_array($route, ['trips','saved','trip-builder','active-trip','trip-mode','past-trips'], true)) wp_enqueue_style('tng-trips-ui', TNG_OS_URL . 'assets/css/trips-ui.css', ['tng-ui-kit'], '0.2.3');
    if (in_array($route, ['saved','trip-builder','active-trip','trip-mode','past-trips'], true)) wp_enqueue_style('tng-trip-builder-ui', TNG_OS_URL . 'assets/css/trip-builder-ui.css', ['tng-trips-ui'], '0.1.2');
    if (in_array($route, ['active-trip','trip-mode'], true)) wp_enqueue_style('tng-active-trip-ui', TNG_OS_URL . 'assets/css/active-trip-ui.css', ['tng-trip-builder-ui'], '0.1.1');
    if ($route === 'past-trips') wp_enqueue_style('tng-past-trips-ui', TNG_OS_URL . 'assets/css/past-trips-ui.css', ['tng-trip-builder-ui'], '0.1.0');
    if ($route === 'profile') wp_enqueue_style('tng-profile-ui', TNG_OS_URL . 'assets/css/profile-ui.css', ['tng-ui-kit'], '0.2.3');
    if ($route === 'profile-settings') wp_enqueue_style('tng-settings-ui', TNG_OS_URL . 'assets/css/settings-ui.css', ['tng-ui-kit'], '0.1.3');
    if ($route === 'search') wp_enqueue_style('tng-search-ui', TNG_OS_URL . 'assets/css/search-ui.css', ['tng-ui-kit'], '0.1.8');
    if (in_array($route, ['leaderboard','achievements'], true)) wp_enqueue_style('tng-progress-ui', TNG_OS_URL . 'assets/css/progress-ui.css', ['tng-ui-kit'], '0.1.7');
    if (in_array($route, ['friends','activity'], true)) wp_enqueue_style('tng-social-ui', TNG_OS_URL . 'assets/css/social-ui.css', ['tng-ui-kit'], '0.1.6');
    if ($route === 'challenges') wp_enqueue_style('tng-challenges-ui', TNG_OS_URL . 'assets/css/challenges-ui.css', ['tng-ui-kit'], '0.1.5');
    if (in_array($route, ['journal','explorer-journal','completed','my-photos'], true)) wp_enqueue_style('tng-library-ui', TNG_OS_URL . 'assets/css/library-ui.css', ['tng-ui-kit'], '0.1.4');
    if (in_array($route, ['trails','events','food','top-sights','destinations'], true)) wp_enqueue_style('tng-directory-ui', TNG_OS_URL . 'assets/css/directory-ui.css', ['tng-ui-kit'], '0.1.9');
    wp_enqueue_script('tng-platform-ui', TNG_OS_URL . 'assets/js/platform-ui.js', [], '2.1.0', true);
    if ($route === 'trip-builder') wp_enqueue_script('tng-trip-builder', TNG_OS_URL . 'assets/js/trip-builder.js', ['tng-trip-data'], '0.1.2', true);
    if (in_array($route, ['active-trip','trip-mode'], true)) wp_enqueue_script('tng-active-trip', TNG_OS_URL . 'assets/js/active-trip.js', ['tng-trip-data'], '0.1.1', true);
}, 100);
