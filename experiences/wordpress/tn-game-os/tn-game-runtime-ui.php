...
final class TNG_Game_Runtime_UI {
    ...
    private static function checkpoints($id) {
        ...
        if (is_array($saved) && !empty($saved)) {
            foreach ($saved as $index => $item) $out[] = self::normalize_checkpoint($item, $index);
        }
        ...
        for ($i = 1; $i <= $count; $i++) {
            $out[] = self::normalize_checkpoint(array(
                'title' => 'Checkpoint '.$i,
    ...
    private static function next_index($checkpoints, $completed) {
        ...
        for ($i = 0; $i < $total; $i++) if (!in_array($i, $completed, true)) return $i;
        return -1;
    }
    ...
    private static function save_photo($id, $index) {
        ...
        $file = $_FILES['checkpoint_photo'];
        if (!empty($file['size']) && (int)$file['size'] > 8 * 1024 * 1024) return false;
        $checked = wp_check_filetype_and_ext($file['tmp_name'], $file['name']);
        if (empty($checked['type']) || strpos($checked['type'], 'image/') !== 0) return false;
        require_once ABSPATH.'wp-admin/includes/file.php';
        require_once ABSPATH.'wp-admin/includes/media.php';
        require_once ABSPATH.'wp-admin/includes/image.php';
        $attachment_id = media_handle_upload('checkpoint_photo', $id, array(), array('test_form' => false));
        if (is_wp_error($attachment_id)) return false;
        update_user_meta(get_current_user_id(), '_tng_game_photo_'.absint($id).'_'.absint($index), absint($attachment_id));
        return true;
    }

    private static function redirect_with_status($id, $error = '') {
        $args = array('game' => absint($id));
        if ($error !== '') $args['runtime_error'] = sanitize_key($error);
        wp_safe_redirect(add_query_arg($args, home_url('/game-play/')));
        exit;
    }
    ...
    public static function handle_action() {
        ...
        if ($action === 'complete') {
            ...
            if ($type === 'question') {
                $given = self::normalize_answer(wp_unslash($_POST['checkpoint_answer'] ?? ''));
                $expected = self::normalize_answer($checkpoint['answer']);
                if ($expected === '' || $given === '' || !hash_equals($expected, $given)) self::redirect_with_status($id, 'answer');
            }

            if ($type === 'gps') {
                $lat = isset($_POST['player_lat']) ? (float)$_POST['player_lat'] : 0.0;
                $lng = isset($_POST['player_lng']) ? (float)$_POST['player_lng'] : 0.0;
                if (!$lat && !$lng) self::redirect_with_status($id, 'location');
                $distance = self::distance_meters($lat, $lng, $checkpoint['latitude'], $checkpoint['longitude']);
                if ($distance > (float)$checkpoint['radius']) self::redirect_with_status($id, 'distance');
            }
        ...
        if ($action === 'reset') {
            ...
            if (is_array($games)) {
                $kept = array();
                foreach ($games as $game_id) { $game_id = absint($game_id); if ($game_id !== $id) $kept[] = $game_id; }
        ...
        self::redirect_with_status($id);
    }

    private static function error_message($code) {
        $messages = array(
            'sequence' => 'Finish the current checkpoint before moving ahead.',
            'answer' => 'That answer is not quite right. Try again.',
            'location' => 'Your location could not be read. Allow location access and try again.',
            'distance' => 'You are not close enough to this checkpoint yet.',
            'photo' => 'The photo could not be uploaded. Try a JPG, PNG, or WebP under 8 MB.',
        );
        return isset($messages[$code]) ? $messages[$code] : '';
    }

    private static function type_label($type) {
        $labels = array('tap'=>'Check in','question'=>'Puzzle','gps'=>'Location','photo'=>'Photo');
        return isset($labels[$type]) ? $labels[$type] : 'Checkpoint';
    }
    ...
    public static function render() {
        ...
        $error = sanitize_key(wp_unslash($_GET['runtime_error'] ?? ''));
            ...
            <div class="tng-runtime-layout"><section class="tng-runtime-list"><div class="tng-section__heading"><div><span class="tng-eyebrow">Game route</span><h2>Checkpoints</h2><p>Your progress is saved automatically to your Explorer account.</p></div></div>
                <?php foreach ($checkpoints as $index => $checkpoint): $is_done=in_array($index,$completed,true); $is_next=(!$finished&&$index===$next); $type=$checkpoint['type']; ?>
                    <article class="tng-runtime-stop<?php echo $is_done?' is-complete':($is_next?' is-next':''); ?>" data-checkpoint-type="<?php echo esc_attr($type); ?>">
                        ...
                        <div class="tng-runtime-stop__copy"><small><?php echo esc_html($is_done?'Completed':($is_next?self::type_label($type):'Locked until previous stop')); ?></small><h3><?php echo esc_html($checkpoint['title']); ?></h3><?php if(!empty($checkpoint['instructions'])):?><p><?php echo esc_html($checkpoint['instructions']); ?></p><?php endif;?>
                            <?php if($is_done&&$type==='photo'): $photo_id=absint(get_user_meta(get_current_user_id(),'_tng_game_photo_'.$id.'_'.$index,true)); if($photo_id):?><div class="tng-runtime-photo-thumb"><?php echo wp_get_attachment_image($photo_id,'thumbnail'); ?></div><?php endif; endif; ?>
                        </div>
                        <?php if($is_next): ?>
                            <div class="tng-runtime-action">
