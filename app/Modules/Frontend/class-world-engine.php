...
final class World_Engine implements Module_Interface {
    ...
    public function admin_page(): void {
        ...
        <div class="wrap tng-world-admin">
            ...
            <div class="tng-world-hero"><p style="color:#f6bd3b;text-transform:uppercase;letter-spacing:.13em;font-weight:800;margin:0 0 8px">TN Platform · Living World</p><h1>World Engine</h1><p>Turn quests, places, events, and discoveries into one location-aware player world.</p><div class="tng-world-actions"><a class="button button-primary button-hero" target="_blank" href="<?php echo esc_url(add_query_arg('tng_world','1',home_url('/'))); ?>">Open World Map</a><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=tng-spatial-integrity')); ?>">Spatial Integrity</a><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=tng-quest-library')); ?>">Quest Library</a></div></div>
            ...
            <div class="tng-world-grid">
                ...
                <div class="tng-world-card"><h2>Foundation status</h2><p><strong style="font-size:16px;color:#067647">Ready</strong></p><p>The first World Engine release includes a unified map, live player location, category filters, nearby sorting, daily challenges, quest markers, and discovery reward previews.</p><p style="color:#667085">Dynamic XP shown in this release is a preview layer. Server-side reward rules and award verification come next.</p></div>
            </div>
        </div>
        <?php
    }
    ...
    public function route(): void {
        ...
        wp_head();
        echo '</head><body class="tng-world-page">'.$this->render().wp_footer().'</body></html>';
        exit;
    }
    ...
    private function render(): string {
        ...
        <main class="tng-world">
            ...
            <div class="tng-world-layout"><section class="tng-world-map-card"><div class="tng-world-map-head"><strong>World map</strong><span data-count></span></div><div class="tng-world-map"></div></section><aside class="tng-world-side"><section class="tng-world-panel"><h2>Nearby</h2><div data-nearby></div></section><section class="tng-world-panel"><h2>Daily challenge</h2><p class="tng-world-meta"><?php echo esc_html(wp_date('F j, Y')); ?> · Complete all three discoveries</p><div data-daily></div><span class="tng-world-reward">500 XP preview</span></section></aside></div>
            <script type="application/json" class="tng-world-data"><?php echo wp_json_encode($payload); ?></script>
            <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
            <script>
            (()=>{const root=document.currentScript.closest('.tng-world'),data=JSON.parse(root.querySelector('.tng-world-data').textContent),mapNode=root.querySelector('.tng-world-map'),nearby=root.querySelector('[data-nearby]'),daily=root.querySelector('[data-daily]'),status=root.querySelector('.tng-world-status'),count=root.querySelector('[data-count]');let map,position=null,user=null,markers=[],filter='all';const esc=v=>String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));const icon=(kind,label)=>L.divIcon({className:'',html:`<div class="tng-world-marker tng-world-marker-${kind}">${esc(label)}</div>`,iconSize:[32,32],iconAnchor:[16,16]});const distance=(a,b)=>{const R=3958.8,p=Math.PI/180,dLat=(b.lat-a.lat)*p,dLon=(b.lng-a.lng)*p,x=Math.sin(dLat/2)**2+Math.cos(a.lat*p)*Math.cos(b.lat*p)*Math.sin(dLon/2)**2;return R*2*Math.atan2(Math.sqrt(x),Math.sqrt(1-x));};const all=()=>[...data.entities.map(x=>({...x,kind:x.type==='event'||x.type==='concert'?'event':'place'})),...data.quests.map(x=>({...x,kind:'quest'}))];const reward=x=>x.kind==='quest'?250:x.kind==='event'?100:25;const draw=()=>{markers.forEach(m=>m.remove());markers=[];const items=all().filter(x=>filter==='all'||x.kind===filter);items.forEach(x=>{const m=L.marker([x.lat,x.lng],{icon:icon(x.kind,x.kind==='quest'?'Q':x.kind==='event'?'E':'•')}).addTo(map).bindPopup(`<strong>${esc(x.title)}</strong><br>${esc(x.label||x.kind)}<br><b>+${reward(x)} XP discovery</b>`);markers.push(m)});count.textContent=items.length+' visible';renderLists(items);};const renderLists=items=>{const sorted=[...items].sort((a,b)=>position?distance(position,a)-distance(position,b):a.title.localeCompare(b.title)).slice(0,6);nearby.innerHTML=sorted.map(x=>`<div class="tng-world-item"><strong>${esc(x.title)}</strong><div class="tng-world-meta">${esc(x.label||x.kind)}${position?' · '+distance(position,x).toFixed(1)+' mi':''}</div><span class="tng-world-reward">+${reward(x)} XP preview</span></div>`).join('')||'<p>No mapped discoveries in this filter.</p>';const lookup=new Map(all().map(x=>[String(x.id),x]));daily.innerHTML=data.daily.map(id=>lookup.get(String(id))).filter(Boolean).map((x,i)=>`<div class="tng-world-item"><strong>${i+1}. ${esc(x.title)}</strong><div class="tng-world-meta">${esc(x.label||x.kind)}</div></div>`).join('')||'<p>Add more mapped entities to generate a daily challenge.</p>';};map=L.map(mapNode).setView([35.7,-85.5],7);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap contributors'}).addTo(map);const bounds=all().map(x=>[x.lat,x.lng]);if(bounds.length===1)map.setView(bounds[0],14);else if(bounds.length>1)map.fitBounds(bounds,{padding:[35,35],maxZoom:14});draw();root.querySelectorAll('[data-filter]').forEach(b=>b.addEventListener('click',()=>{filter=b.dataset.filter;root.querySelectorAll('[data-filter]').forEach(x=>x.classList.toggle('is-active',x===b));draw()}));root.querySelector('[data-locate]').addEventListener('click',()=>{if(!navigator.geolocation){status.textContent='Location is unavailable';return;}status.textContent='Locating…';navigator.geolocation.watchPosition(p=>{position={lat:p.coords.latitude,lng:p.coords.longitude};status.textContent='Live · accuracy ±'+Math.round(p.coords.accuracy*3.28084)+' ft';if(!user)user=L.marker([position.lat,position.lng],{icon:icon('you','')}).addTo(map).bindPopup('Your location');else user.setLatLng([position.lat,position.lng]);map.setView([position.lat,position.lng],13);draw();},e=>status.textContent=e.code===1?'Location permission denied':'Location unavailable',{enableHighAccuracy:true,maximumAge:5000,timeout:15000})});setTimeout(()=>map.invalidateSize(),250);})();
            </script>
        </main>
    ...
    private function entities(): array {
        ...
        $items=[];
        foreach($posts as $post){$payload=(array)get_post_meta($post->ID,'_tng_entity_payload',true);$coords=$this->coords($payload);if(!$coords)continue;$type=sanitize_key((string)get_post_meta($post->ID,'_tng_entity_type',true))?:'place';$items[]=['id'=>(string)get_post_meta($post->ID,'_tng_entity_id',true),'title'=>$post->post_title,'type'=>$type,'label'=>$this->label($type),'lat'=>$coords[0],'lng'=>$coords[1]];}
        return $items;
    }

    private function quests(): array {
        $posts=get_posts(['post_type'=>self::QUEST_TYPE,'post_status'=>'publish','posts_per_page'=>200]);$items=[];
        foreach($posts as $post){$ids=(array)get_post_meta($post->ID,'_tng_quest_entity_ids',true);$lat=null;$lng=null;foreach($ids as $entity_id){$entity=get_posts(['post_type'=>self::ENTITY_TYPE,'post_status'=>['publish','draft','private'],'meta_key'=>'_tng_entity_id','meta_value'=>(string)$entity_id,'posts_per_page'=>1]);if(!$entity)continue;$coords=$this->coords((array)get_post_meta($entity[0]->ID,'_tng_entity_payload',true));if($coords){[$lat,$lng]=$coords;break;}}if($lat===null)continue;$items[]=['id'=>'quest-'.$post->ID,'title'=>$post->post_title,'label'=>'Quest','lat'=>$lat,'lng'=>$lng,'url'=>add_query_arg('tng_quest_runtime_id',$post->ID,home_url('/'))];}
        return $items;
    }

    private function daily_ids(array $entities): array {
        if (!$entities) return [];
        $copy=$entities;mt_srand((int)wp_date('Ymd'));shuffle($copy);mt_srand();return array_map(static fn($x)=>(string)$x['id'],array_slice($copy,0,3));
    }

    private function coords(array $p): ?array {$lat=$p['latitude']??$p['lat']??null;$lng=$p['longitude']??$p['lng']??$p['lon']??null;if((!is_numeric($lat)||!is_numeric($lng))&&isset($p['coordinates'])&&is_array($p['coordinates'])){$lat=$p['coordinates']['lat']??null;$lng=$p['coordinates']['lng']??null;}return is_numeric($lat)&&is_numeric($lng)?[(float)$lat,(float)$lng]:null;}
    private function label(string $type): string {return ucwords(str_replace(['-','_'],' ',$type?:'place'));}
}
